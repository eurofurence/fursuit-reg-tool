...
beforeEach(function () {
    ...
    $this->props = fn (array $query = []) => get(route('admin.staff.index', $query))
        ->viewData('page')['props'];
});
...
test('a guest is redirected to login', function () {
    get(route('admin.staff.index'))->assertRedirect(route('login'));
});
...
test('a reviewer holds access-manage but is refused every staff ability', function () {
    ...
    get(route('admin.staff.index'))->assertForbidden();
    get(route('admin.staff.create'))->assertForbidden();
...
test('a reviewer is refused every rfid tag endpoint', function () {
    ...
    post(route('admin.staff.rfid-tags.store', $staff), manageRfidPayload(['content' => '9']))->assertForbidden();
    put(route('admin.staff.rfid-tags.update', [$staff, $tag]), manageRfidPayload())->assertForbidden();
    delete(route('admin.staff.rfid-tags.destroy', [$staff, $tag]))->assertForbidden();
    delete(route('admin.staff.rfid-tags.bulk.destroy', $staff), ['ids' => [$tag->id]])->assertForbidden();
...
test('the PIN column is the two literal words and the PIN itself never reaches the payload', function () {
    ...
    $response = get(route('admin.staff.index'));
...
test('the edit page never carries the plaintext PIN either', function () {
    ...
    $response = get(route('admin.staff.edit', $staff));
...
test('a member with no PIN gets an empty field rather than the sentinel', function () {
    ...
    expect(get(route('admin.staff.edit', $staff))->viewData('page')['props']['staff']['pin_code'])
        ->toBe('');
});
...
test('submitting the sentinel unchanged keeps the stored PIN', function () {
    ...
    put(route('admin.staff.update', $staff), manageStaffPayload([
        'name' => 'Steady Skunk',
...
test('the sentinel itself can never be written as a PIN', function () {
    ...
    post(route('admin.staff.store'), manageStaffPayload([
...
test('the page action is New staff', function () {
    ...
    expect($actions)->toHaveCount(1)
        ...
        ->and($actions[0]['url'])->toBe(route('admin.staff.create'))
...
test('the name is required and the PIN must be exactly six digits', function () {
    ...
    post(route('admin.staff.store'), manageStaffPayload(['name' => '']))
...
test('a weak PIN is refused by SecurePinRule', function () {
    ...
    post(route('admin.staff.store'), manageStaffPayload(['pin_code' => '111111']))
        ->assertSessionHasErrors('pin_code');
});
...
test('another members PIN is still refused, with the deliberately vague message', function () {
    ...
    put(route('admin.staff.update', $other), manageStaffPayload(['pin_code' => '408271']))
...
test('a blank setup code stores null, so a second blank member does not collide', function () {
    ...
    post(route('admin.staff.store'), manageStaffPayload(['name' => 'First Blank']))
        ->assertSessionHasNoErrors();
    post(route('admin.staff.store'), manageStaffPayload(['name' => 'Second Blank']))
        ->assertSessionHasNoErrors();
...
test('a setup code is uppercased on save and cannot duplicate another one', function () {
    ...
    post(route('admin.staff.store'), manageStaffPayload(['name' => 'Upper Otter', 'setup_code' => 'ab12cd']))
        ->assertSessionHasNoErrors();
...
test('Generate proposes a setup code without writing one', function () {
    ...
    $code = session('admin.staff.generated_setup_code');
...
test('the proposed code reaches the form as a prop and only persists on save', function () {
    ...
    post(route('admin.staff.setup-code', $staff));

    $code = get(route('admin.staff.edit', $staff))
...
test('Generate works on the create screen, where there is no record', function () {
    ...
    post(route('admin.staff.setup-code.create'))->assertRedirect();

    expect(session('admin.staff.generated_setup_code'))->toBeString()->toHaveLength(6);
});
...
test('the tag table is the audit column list and shows only this members tags', function () {
    ...
    $props = get(route('admin.staff.edit', $staff))->viewData('page')['props'];
...
test('the tag columns carry the audit labels, placeholders and copyable code', function () {
    ...
    $columns = collect(get(route('admin.staff.edit', $staff))->viewData('page')['props']['columns'])
        ->keyBy('key');
...
test('the tag table filters and sorts through the partial visit', function () {
    ...
    $url = route('admin.staff.edit', $staff, false);
...
test('a tag row offers Delete with Filament default delete copy, and the bulk action names rfid tags', function () {
    ...
    expect($delete['method'])->toBe('delete')
        ->and($delete['url'])->toBe(route('admin.staff.rfid-tags.destroy', [$staff, $tag]))
        ->and($delete['confirm'])->toBe([
    ...
    expect($props['bulkActions'])->toHaveCount(1)
        ...
        ->and($props['bulkActions'][0]['url'])->toBe(route('admin.staff.rfid-tags.bulk.destroy', $staff))
...
test('a tag is created against the owning member', function () {
    ...
    post(route('admin.staff.rfid-tags.store', $staff), manageRfidPayload())
        ->assertSessionHasNoErrors()
        ->assertSessionHas(MANAGE_STAFF_TOAST_TITLE, 'Created');
...
test('a tag code is required and unique across every member', function () {
    ...
    post(route('admin.staff.rfid-tags.store', $staff), manageRfidPayload(['content' => '']))
        ->assertSessionHasErrors('content');

    post(route('admin.staff.rfid-tags.store', $staff), manageRfidPayload(['content' => 'TAKEN']))
        ->assertSessionHasErrors('content');
});
...
test('a tag keeps its own code when it is saved unchanged', function () {
    ...
    put(route('admin.staff.rfid-tags.update', [$staff, $tag]), manageRfidPayload([
        'content' => 'SAME-CODE',
        'name' => 'Renamed',
        'is_active' => false,
    ]))
...
test('a tag is hard deleted, one at a time and in bulk', function () {
    ...
    delete(route('admin.staff.rfid-tags.destroy', [$staff, $one]))
        ->assertSessionHas(MANAGE_STAFF_TOAST_TITLE, 'Deleted');

    assertDatabaseMissing('rfid_tags', ['id' => $one->id]);

    delete(route('admin.staff.rfid-tags.bulk.destroy', $staff), ['ids' => [$two->id, $three->id]])
        ->assertSessionHas(MANAGE_STAFF_TOAST_TITLE, 'Deleted');
...
test('a tag belonging to another member cannot be reached through this one', function () {
    ...
    delete(route('admin.staff.rfid-tags.destroy', [$staff, $theirs]))->assertNotFound();
    put(route('admin.staff.rfid-tags.update', [$staff, $theirs]), manageRfidPayload())->assertNotFound();
...
test('a bulk delete that names a tag of another member deletes nothing', function () {
    ...
    delete(route('admin.staff.rfid-tags.bulk.destroy', $staff), ['ids' => [$mine->id, $theirs->id]])
