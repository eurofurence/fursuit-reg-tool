 ...
 *
 * The fields are the old staff list's form schema with three fixes the plan
 * mandates, all of them about the same thing: this form cannot be saved twice today.
 *
 *  - `SecurePinRule` is handed the record id. the old panel
 ...
 *  - A blank `setup_code` dehydrates to null, not `''`.
 ...
 *    sentinel, which is dropped here before validation. Emptying the
 ...
 *
 * The PIN is validated with `digits:6` rather than the old panel's `numeric` + `length(6)`.
 * Both express "exactly six digits", but `numeric` made the client coerce the value to a
 * number and drop a leading zero, and under a `numeric` rule Laravel's size
 ...
 * comment "don't reveal that another user has this PIN for security"; a
...
class StaffRequest extends FormRequest
{
    ...
    /**
     * the old panel's `->mutateDehydratedStateUsing(fn ($state) => strtoupper($state ?? ''))`,
