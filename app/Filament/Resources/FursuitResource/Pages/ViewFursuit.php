    ...
    protected function getHeaderActions(): array
    {
        ...
        return [
            ...
            Actions\Action::make('Approve')
                ...
                ->visible(fn (Fursuit $record) => $record->status->canTransitionTo(Approved::class))
            ...
            Actions\Action::make('Reject')
                ...
                ->form([
                    Select::make('reason')
                        ->live()
                        ->afterStateUpdated(fn (Set $set, ?string $state) => $set('custom_reason', $errorOptions[$state]))
                        ->options($errorOptions),
                    Textarea::make('custom_reason')
                        ->label('Reason Sent to the User!')
                        ->required(),
                ])
                ->visible(fn (Fursuit $record) => $record->status->canTransitionTo(Rejected::class))
                ->action(function (Fursuit $record) {
