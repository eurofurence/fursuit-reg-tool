<script setup>
/**
 * The printer list, successor to PrinterResource's table and its ListPrinters page.
 *
 ...
 *
 * There is no filter set: PrinterResource declares `->filters([ // ])`. DataTable still
 * gets `searchable`, so its toolbar renders with the search box alone, which the resource
 * hid at table level with `->searchable(false)` and which the name column needs (plan 2.3).
 */
 ...
 * while a printer sat stopped (plan 2.10 #26, audit 7.1). Fifteen seconds, and only the
