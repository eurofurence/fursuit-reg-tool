...
const rarityConfig = computed(() => {
    ...
    const config = {
        ...
        uncommon: {
            ...
            cardRing: "",
            badgeClass:
                "bg-emerald-600 text-white border-emerald-400/30 shadow-emerald-200/50",
        },
        rare: {
            ...
            cardRing: "",
            badgeClass:
                "bg-blue-600 text-white border-blue-400/30 shadow-blue-200/50",
        },
        epic: {
            ...
            cardGlow:
                "shadow-violet-400/40 hover:shadow-violet-400/60 epic-glow",
            cardRing: "ring-1 ring-violet-400/30",
            badgeClass:
                "bg-gradient-to-br from-violet-600 to-violet-700 text-white border-violet-400/40 shadow-violet-300/60",
        },
        legendary: {
            ...
            cardGlow:
                "shadow-amber-400/50 hover:shadow-amber-400/70 legendary-glow",
            cardRing: "ring-1 ring-amber-400/30",
            badgeClass:
                "bg-gradient-to-br from-amber-500 to-orange-600 text-white border-amber-300/50 shadow-amber-300/70",
        },
    };
    ...
    >
        ...
        >
            <div class="absolute bottom-0 left-0 right-0 p-4 text-white">
                <h3
                    class="text-lg font-bold mb-1 transform translate-y-2 group-hover:translate-y-0 transition-transform duration-300 drop-shadow-lg"
                >
                    {{ fursuit.name }}
                </h3>
                <p
                    class="text-sm opacity-90 transform translate-y-2 group-hover:translate-y-0 transition-transform duration-300 delay-75 drop-shadow-md"
                >
                    {{ fursuit.species }}
                </p>
            </div>
        ...
        <div
            class="absolute bottom-0 left-0 right-0 backdrop-blur-sm p-2 transform translate-y-0 group-hover:translate-y-full transition-transform duration-300"
            :class="[rarityConfig.bgClass, rarityConfig.shadowClass]"
        >
            <h4 class="text-sm truncate" :class="[rarityConfig.titleClass]">
                {{ fursuit.name }}
            </h4>
            <p class="text-xs truncate" :class="[rarityConfig.speciesClass]">
                {{ fursuit.species }}
            </p>
        </div>
        ...
        <!-- Catches count -->
        <div
            v-show="fursuit.scoring > 0 && !hideCount"
...
@keyframes epic-pulse {
    ...
    100% {
        box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1),
            0 2px 4px -1px rgba(0, 0, 0, 0.06), 0 0 0 rgba(139, 92, 246, 0);
    }
    50% {
        box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1),
            0 2px 4px -1px rgba(0, 0, 0, 0.06), 0 0 20px rgba(139, 92, 246, 0.3);
    }
}
...
@keyframes legendary-pulse {
    ...
    100% {
        box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1),
            0 4px 6px -2px rgba(0, 0, 0, 0.05), 0 0 0 rgba(251, 191, 36, 0);
    }
    50% {
        box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1),
            0 4px 6px -2px rgba(0, 0, 0, 0.05), 0 0 25px rgba(251, 191, 36, 0.4),
