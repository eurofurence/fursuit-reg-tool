...
const control =
  'h-7 rounded border border-hairline bg-mg-surface-2 px-2 text-[12px] text-fg-1 outline-none focus:border-state-live/50';
</script>

<template>
  <div class="relative flex h-11 flex-wrap items-center gap-2 border-b border-hairline bg-mg-surface-1 px-3">
    ...
    <template v-for="filter in filters" :key="filter.key">
      ...
      <div v-else-if="filter.type === 'select'" class="relative">
        ...
        <div
          ...
          class="absolute top-8 left-0 z-30 w-48 rounded border border-hairline bg-mg-surface-2 p-1.5 shadow-lg"
        >
          <label
          ...
          <button
            type="button"
            class="mt-1 w-full rounded px-1.5 py-1 text-left text-[11px] text-fg-3 transition-colors hover:bg-mg-surface-3 hover:text-fg-1"
      ...
      <label
        ...
        :class="filter.value ? 'border-state-live/40 bg-state-live/10 text-state-live' : 'text-fg-2 hover:bg-mg-surface-3'"
      >
        <input
          type="checkbox"
          class="accent-state-live"
