...
<template>
  <div class="flex h-10 items-center gap-3 border-t border-hairline px-3 text-[12px] text-fg-2">
    ...
    <label class="flex items-center gap-1.5">
      ...
      <select
        :value="meta.perPage"
        class="h-6 rounded border border-hairline bg-mg-surface-2 px-1 text-[12px] text-fg-1"
    ...
    <div v-if="meta.lastPage > 1" class="ml-auto flex items-center gap-1">
      ...
      <button
        ...
        :class="page === meta.page ? 'border-state-live/40 bg-state-live/10 text-state-live' : 'border-hairline hover:bg-mg-surface-3'"
        @click="setPage(page)"
      >
        {{ page }}
      </button>
